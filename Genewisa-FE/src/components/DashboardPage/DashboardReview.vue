...
function getData() {
    axios({
        method: "get",
        url: "http://localhost:8000/api/review?page=" + store.pageNow + "&key=" + store.keywordNow,
        params: {
            token: store.token
        }
    })
...
function deleteData() {
    axios({
        method: "delete",
        url: "http://localhost:8000/api/review/" + store.deletedObject,
        params: {
            token: store.token
        }
    })
    ...
    <table class="table-fixed w-full">
        ...
        <tbody id="data">
            <tr class="h-12" v-for="(data, index) in listData" :data="data" v-bind:class="{'bg-slate-50' : !(index % 2)}">
                ...
                <td> {{ data.comment }}</td>
                <td>
                    <button class="mx-4" @click="confirmDelete(data.id.toString())">
